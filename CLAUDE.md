...
- **Every destination is still a hand-placed instance in `Scenes/World.tscn`.** The strategic layer is real data (`Data/destinations.json` + `DestinationCatalog`) and both destination kinds are real scenes now (`Derelict.tscn`, `Station.tscn`), but nothing is instantiated from that data at runtime — a sixth destination still costs a scene edit plus three `NodePath` array entries on the travel console. `docs/architecture/space-and-travel.md` splits the remaining work into "instantiate from data at startup" (the payoff, self-contained) and "free the absent destination" (needs sim ownership moved off `ShipSim` first, or it undoes the coarse LOD tick).
- **Before any `.tscn` surgery, digest the scene before and after** with `Tools/scene_digest.gd` and diff — nothing in the test suite loads `World.tscn`, and `--headless --quit` won't catch a mesh pointing at the wrong `SubResource` of the right type.
