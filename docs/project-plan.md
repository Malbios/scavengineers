...
- **Atmosphere = lumped per-cell scalars with neighbor-to-neighbor diffusion, never per-tile CFD.** Each cell holds its own scalars (pressure, O₂ fraction, temperature); a breach vents only the directly-exposed cell toward vacuum, and diffusion (each cell moving toward the average of its own unsealed neighbors, one hop per tick) carries the effect outward, so distance/graph-hop-count genuinely matters — a cell several hops from a breach keeps some air for a moment rather than every cell in a room updating in lockstep. Still cheap, time-warp-stable, reuses the room flood-fill, and fully sells the fantasy — no velocity/momentum/advection, still not a fluid sim. This ceiling is still deliberate — do not let it "improve" into real fluid dynamics.
...
- **Atmosphere:** lumped per-cell scalars with neighbor-to-neighbor diffusion, no CFD (deliberate ceiling).
