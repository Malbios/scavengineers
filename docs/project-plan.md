...
- **Atmosphere = lumped per-cell scalars, never per-tile CFD.** Each cell holds its own scalars (pressure, O₂ fraction, temperature). A breach (or an open airlock bridging to a room that has one) vents its entire connected-to-outside component uniformly toward vacuum, at the same rate for every cell — matching real depressurization, where internal pressure equalizes at the speed of sound, vastly faster than air escapes through a hole, so a connected volume never develops a distance-based gradient in practice. Neighbor-to-neighbor diffusion still exists for sealed, non-vented components (two rooms mixing through an open door, no external vacuum driving force) but never runs for a vented one. Still cheap, time-warp-stable, reuses the room flood-fill, and fully sells the fantasy — no velocity/momentum/advection, still not a fluid sim. This ceiling is still deliberate — do not let it "improve" into real fluid dynamics.
...
- **Atmosphere:** lumped per-cell scalars, uniform whole-component venting once breached, no CFD (deliberate ceiling).
