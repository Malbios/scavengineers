...
- **`scene-authoring.md`** — craft knowledge for `.tscn` edits and node startup order, plus the two headless tools (`Tools/scene_digest.gd`, `Tools/world_smoke.gd`) that make scene refactors verifiable at all. Read it before touching a scene file; nothing in the test suite loads `World.tscn`.
